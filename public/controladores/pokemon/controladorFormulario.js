...
function enviardatos() {
    ...
    if (errorcamposvacios) {
        ...
        const apelldio2 = inputapellido2.value;
        const usuario = inputusuario.value;
        const email = inputemail.value;
        const fechanacimiento = inputfechanacimiento.value;
        const cedula = inputidentificacion.value;
        const tipocedula = document.querySelector('input[name="tipoIdentificacion"]:checked').value;
        const foto = inputfoto.src;

        registrarUsuario(nombre, apellido1, apelldio2, usuario, email, fechanacimiento, cedula, tipocedula, foto)
